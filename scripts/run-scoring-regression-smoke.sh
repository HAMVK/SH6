...
PORT="${SCORING_SMOKE_PORT:-8782}"
URL="http://127.0.0.1:${PORT}/tests/scoring-regression-smoke.html"
SESSION="sh6-scoring-smoke-${RANDOM}-${RANDOM}"
SERVER_LOG="${SCORING_SMOKE_SERVER_LOG:-/tmp/sh6-scoring-smoke-server.log}"
...
echo "[scoring-smoke] Starting static server at ${URL}"
...
if ! kill -0 "${HTTP_PID}" >/dev/null 2>&1; then
  echo "[scoring-smoke] Failed to start local server. See ${SERVER_LOG}" >&2
  exit 1
fi
...
echo "[scoring-smoke] status=${status}"
echo "${result_json}"

if [[ "${status}" != "PASS" ]]; then
  echo "[scoring-smoke] Scoring regression smoke test failed." >&2
  exit 1
fi

echo "[scoring-smoke] PASS"
